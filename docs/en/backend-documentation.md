...
- **`Vacancy`** — an active opening tied to a Position: `title`, `availableSlots`, `startDate`/`endDate`, `status` (default `ACTIVE`). Cascades on Position delete and User delete. **Since 2026-07-23**, `status` also gets set to `CLOSED` automatically once `availableSlots` fills up with `SELECCIONADO` applications — see the `Application` entry below and [§8.6 of `api-documentation.md`](./api-documentation.md#86-vacancy-slot-enforcement--auto-close-added-2026-07-23) for the full rule.
...
- **`Application`** — join table linking a Candidate to a Vacancy it applied to (`status: ApplicationStatus`, `@@unique([candidateId, vacancyId])`, relations to `Candidate`/`Vacancy` are `onDelete: Cascade`). Actively written by the upload pipeline (see [§11](#11-the-cv-processing-pipeline)) and read by `POST /api/vacancies/:id/evaluations` to source pending candidates. **Since 2026-07-23**, `status` is also writable directly via `PATCH /api/vacancies/:vacancyId/candidates/:candidateId/status`, which enforces `Vacancy.availableSlots` (rejects a new `SELECCIONADO` once slots are full, and closes the vacancy in the same transaction when the last slot is taken — never reopens it automatically). There is still **no generic `/api/applications` CRUD endpoint** for `Application` rows by their own id; this write path is scoped to one candidate within one vacancy.
...
- **`Application` model has no generic CRUD endpoint:** actively written/read internally by the upload and evaluation pipeline (see [§11](#11-the-cv-processing-pipeline)), exposed read-only (nested `ApplicationStatus`) via `GET /api/vacancies/:id/results`, and — since 2026-07-23 — writable via `PATCH /api/vacancies/:vacancyId/candidates/:candidateId/status` (see `api-documentation.md` §8.6). There is still no `/api/applications` route to manage `Application` rows by their own id.
- **Vacancy slot enforcement is intentionally minimal, not built for high concurrency:** the row lock used to serialize concurrent hires (`SELECT ... FOR UPDATE` on the `Vacancy` row inside `PATCH /api/vacancies/:vacancyId/candidates/:candidateId/status`) only protects a single vacancy's own hiring decisions from racing each other — it is a stopgap for the "vacancy never auto-closes" bug, not a scalable slot-reservation system. A closed vacancy never reopens automatically (neither by freeing a slot nor by raising `availableSlots`); reopening is always a manual `PATCH /api/vacancies/:id/status`.
