...
- **`Vacancy`** — una apertura activa ligada a una Posición: `title`, `availableSlots`, `startDate`/`endDate`, `status` (default `ACTIVE`). Cascadea al borrar la Posición y al borrar el Usuario. **Desde 2026-07-23**, `status` también pasa a `CLOSED` automáticamente cuando `availableSlots` se llena con `Application` en `SELECCIONADO` — ver la entrada de `Application` abajo y el [§8.6 de `api-documentation.md`](./api-documentation.md#86-cupos-de-la-vacante-y-auto-cierre-agregado-2026-07-23) para la regla completa.
...
- **`Application`** — tabla de unión que enlaza un Candidato con una Vacante a la que aplicó (`status: ApplicationStatus`, `@@unique([candidateId, vacancyId])`, relaciones a `Candidate`/`Vacancy` son `onDelete: Cascade`). Escrita activamente por el pipeline de subida (ver [§11](#11-el-pipeline-de-procesamiento-de-cvs)) y leída por `POST /api/vacancies/:id/evaluations` para obtener los candidatos pendientes. **Desde 2026-07-23**, `status` también es escribible directamente vía `PATCH /api/vacancies/:vacancyId/candidates/:candidateId/status`, que aplica el límite de `Vacancy.availableSlots` (rechaza un nuevo `SELECCIONADO` cuando los cupos ya están llenos, y cierra la vacante en la misma transacción cuando se ocupa el último cupo — nunca la reabre automáticamente). Sigue sin existir un CRUD genérico `/api/applications` para filas `Application` por su propio id; esta vía de escritura está acotada a un candidato dentro de una vacante.
...
- **El modelo `Application` no tiene endpoint CRUD genérico:** escrito/leído activamente de forma interna por el pipeline de subida y evaluación (ver [§11](#11-el-pipeline-de-procesamiento-de-cvs)), expuesto en modo lectura (`ApplicationStatus` anidado) vía `GET /api/vacancies/:id/results`, y — desde 2026-07-23 — escribible vía `PATCH /api/vacancies/:vacancyId/candidates/:candidateId/status` (ver `api-documentation.md` §8.6). Sigue sin existir una ruta `/api/applications` para gestionar filas `Application` por su propio id.
- **El bloqueo de cupos de la vacante es deliberadamente mínimo, no pensado para alta concurrencia:** el lock de fila usado para serializar contrataciones concurrentes (`SELECT ... FOR UPDATE` sobre la fila `Vacancy` dentro de `PATCH /api/vacancies/:vacancyId/candidates/:candidateId/status`) solo protege las decisiones de contratación de una misma vacante entre sí — es un parche para el bug de "la vacante nunca se auto-cierra", no un sistema escalable de reserva de cupos. Una vacante cerrada nunca se reabre automáticamente (ni liberando un cupo ni subiendo `availableSlots`); reabrir siempre es un `PATCH /api/vacancies/:id/status` manual.
