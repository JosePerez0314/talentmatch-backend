...
9. **Documentation Sync (Ask First):** The project documentation lives in `docs/`, in both English (`docs/en/`) and Spanish (`docs/es/`): the full architecture reference (`backend-documentation.md`), the endpoint contract (`api-documentation.md`), the READMEs, the frontend changelog, the recent-changes log, and the context/persona prompts (`context/`). Whenever a change touches **backend behavior** (routes, controllers, services, middlewares, validation, error handling, the CV pipeline, the scoring engine) OR the **database** (`prisma/schema.prisma`, migrations, enums, indexes), you MUST ask me whether to update the affected documentation before treating the task as complete. If I approve, update **both** the English and Spanish versions so they never drift apart, and keep them consistent with the code. Do not let the docs silently fall out of sync with the backend or the schema. (Note: `docs/index.html` and `docs/styles/` are auto-generated by `prisma-docs-generator` on `npx prisma generate` — never hand-edit them.)

## Git & Commit Formatting

10. **Strict Commit Format (Conventional Commits):** Whenever I explicitly instruct you to generate a commit message or commit a specific set of changes, you MUST strictly adhere to the Conventional Commits specification.
